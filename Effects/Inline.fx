...
float4 InlineShader(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
	...
	float inlineLine = (1 - (pixelLeft2 * pixelUp2 * pixelRight2 * pixelDown2)) * color.a;
	// float inlineLine = (1 - (pixelLeft2 * pixelUp2 * pixelRight2 * pixelDown2)) * color.a;
	
	// Figure out if it is the outside of the texture.
	// float inlineLine = max(max(pixelLeft2, pixelUp2), max(pixelRight2, pixelDown2)) - color.a;

	// Create a new color for the outline color.
	float4 inlineColor = float4(1, 1, 1, 1);
	inlineColor.rgb *= uColor; // multiply it by the color passed through the dye color argument.
	...
	return lerp(color * sampleColor, inlineColor, inlineLine);
	/*if (inlineLine >= 1)
	{
		return inlineColor;
	}
...
technique Technique1
{
	pass InlinePass
	{
		PixelShader = compile ps_2_0 InlineShader();
	}
}