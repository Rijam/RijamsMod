    ...
    public class GlobalTiles : GlobalTile
    {
		public override void Drop(int i, int j, int type)
		{
            if (type == TileID.Pots)
            {
                ...
                {
                    Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i * 16, j * 16), ModContent.ItemType<Items.Weapons.Ranged.Ammo.BloodyArrow>(), Main.rand.Next(20, 41));
                }
				...
				if (j > Main.rockLayer && j < Main.UnderworldLayer)
				{
					...
					{
						Item.NewItem(new EntitySource_TileBreak(i, j), new Vector2(i * 16, j * 16), ModContent.ItemType<Items.Consumables.CaveCarrot>());
					}
