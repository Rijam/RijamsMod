...
{
	public class BabyBoneSerpentStaff : ModItem
	{
		public override void SetStaticDefaults()
		{
			ItemOriginDesc.itemList.Add(Item.type, ["[c/474747:Dropped from Bone Serpents]"]);
		...
		public override void SetDefaults()
		{
			...
			Item.buffType = ModContent.BuffType<BabyBoneSerpentBuff>();
			// No buffTime because otherwise the item tooltip would say something like "1 minute duration"
			Item.shoot = ModContent.ProjectileType<BabyBoneSerpent>();
		}
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			...
			if (!player.HasBuff(ModContent.BuffType<BabyBoneSerpentBuff>()))
			{
				...
				{
					ModContent.GetInstance<RijamsMod>().Logger.Error($"Failed to spawn Baby Bone Serpent. {e}");
				}
			...
			{
				if (projectile.type == ModContent.ProjectileType<BabyBoneSerpent>() && projectile.ai[0] < player.maxMinions && projectile.owner == Main.myPlayer)
				{
					...
					if (Main.netMode == NetmodeID.SinglePlayer)
					{
						...
						{
							ModContent.GetInstance<RijamsMod>().Logger.Warn($"Baby Bone Serpent Staff: unable to resize oldPos array. {e}");
						}
					}
					projectile.netUpdate = true;
				}
