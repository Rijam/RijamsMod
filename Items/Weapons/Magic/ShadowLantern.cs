	...
	public class ShadowLantern : ModItem
	{
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			for (int i = 0; i < Item.useLimitPerAnimation; i++)
			{
				...
				Vector2 randomCircular = Main.rand.NextVector2Circular(1f, 1f) + Main.rand.NextVector2CircularEdge(5f, 5f);
				...
				if (projectile.ModProjectile is LanternLight modProjectile)
				{
					...
					modProjectile.timeLeftBeforeItStopsHoming = 20;
		...
		public override void HoldItem(Player player)
		{
			...
			Lighting.AddLight(playerPos, new Vector3(0.2f, 0.3f, 0.32f));
			if (Main.rand.NextBool(40))
			{
				Vector2 randomCirclular = Main.rand.NextVector2Circular(4f, 4f);
				Dust dust = Dust.NewDustPerfect(playerPos + randomCirclular, DustID.TintableDustLighted, Vector2.Zero, 254, new Color(0.2f, 0.3f, 0.32f, 1f), 0.3f);
		...
		{
			Lighting.AddLight(Item.Center, new Vector3(0.2f, 0.3f, 0.32f));
		}
