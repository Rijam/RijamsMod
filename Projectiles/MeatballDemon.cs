	...
	public class MeatballDemon : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (player.dead || !player.active) {
				...
				player.ClearBuff(mod.BuffType("MeatballDemonBuff"));
			}
			//if (player.HasBuff(ModContent.BuffType<HissyDemonBuff>()))
			if (player.HasBuff(mod.BuffType("MeatballDemonBuff")))
			{
			...
			if (projectile.ai[1] == 0f)
			{
				...
				if (Main.myPlayer == projectile.owner)
				{
					...
					vector11.Normalize();
					vector11 *= num30;
					int num36 = Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, vector11.X, vector11.Y, ModContent.ProjectileType<LostSkull>(), projectile.damage, 0f, Main.myPlayer);
					Main.projectile[num36].timeLeft = 300;
					Main.projectile[num36].netUpdate = true;
					projectile.netUpdate = true;
					if (!Main.dedServ)
					{
						Main.PlaySound(mod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/MeatballDemonShoot").WithVolume(0.8f), (int)projectile.position.X, (int)projectile.position.Y);
					}
				}
			}
		}
