...
namespace RijamsMod.Projectiles.Summon.Support
{
	public class CrystalCluster : ModProjectile
	{
        ...
        public override void AI()
		{
			...
			{
				player.ClearBuff(ModContent.BuffType<Buffs.Minions.CrystalClusterBuff>());
			}
			if (player.HasBuff(ModContent.BuffType<Buffs.Minions.CrystalClusterBuff>()))
			{
			...
			int radius = (distRadius + player.GetModPlayer<RijamsModPlayer>().supportMinionRadiusIncrease) * 16; // 15 tiles
			if (Main.netMode != NetmodeID.Server)
			{
				...
				if (configClient.DisplayDefenseSupportSummonsAura != RijamsModConfigClient.SupportSummonsAura.Off)
				{
					for (int i = 0; i < 60; i++)
					{
						...
						Dust d = Dust.NewDustPerfect(Projectile.Center + speed * radius, ModContent.DustType<Dusts.AuraDust>(), speed, alpha, Color.Magenta, 0.75f);
						d.noGravity = true;
