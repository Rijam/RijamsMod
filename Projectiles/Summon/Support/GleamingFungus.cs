...
namespace RijamsMod.Projectiles.Summon.Support
{
	public class GleamingFungus : ModProjectile
	{
        ...
        public override void AI()
		{
			...
			{
				player.ClearBuff(ModContent.BuffType<Buffs.Minions.GleamingFungusBuff>());
			}
			if (player.HasBuff(ModContent.BuffType<Buffs.Minions.GleamingFungusBuff>()))
			{
			...
			if (Main.netMode != NetmodeID.Server)
			{
				...
				if (configClient.DisplayHealingSupportSummonsAura != RijamsModConfigClient.SupportSummonsAura.Off)
				{
					for (int i = 0; i < 50; i++)
					{
						...
						Dust d = Dust.NewDustPerfect(Projectile.Center + speed * radius, ModContent.DustType<Dusts.AuraDust>(), speed, alpha, Color.DeepSkyBlue, 0.75f);
						d.noGravity = true;
		...
		public override void PostDraw(Color lightColor)
		{
			...
			Main.EntitySpriteDraw(glowTexture, Projectile.Center - Main.screenPosition + new Vector2(0, Projectile.gfxOffY + 1), sourceRectangle, Projectile.GetAlpha(lightColor), Projectile.rotation, sourceRectangle.Size() / 2f, Projectile.scale, spriteEffects);
		}
