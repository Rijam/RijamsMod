...
{
    public class GiantWorm : ModProjectile
    {
		...
		public override void AI()
        {
			...
			{
				player.ClearBuff(ModContent.BuffType<Buffs.Minions.GiantWormBuff>());
			}
			if (player.HasBuff(ModContent.BuffType<Buffs.Minions.GiantWormBuff>()))
			{
			...
			*/
			...
			if (target != -1) // Has a target
			{
				...
				{
					minVelocity += ((distFromTarget - 32) / 32f) + Math.Min(NumberOfTimesSummoned / 2f, 4f); // Speed up if further away from the target.
		...
		public static void HoverAroundPlayer(Projectile projectile, Player player, ref float minVelocity)
		{
			...
			minVelocity = Math.Min(8f, Math.Max(4f, player.velocity.Length())); // Get faster if the player is moving fast.
		...
		public override bool PreDraw(Player player, ref Color lightColor)
		{
			...
			for (int i = 1; i < numberOfSegmentsToDraw; i++) // Draw the number of times summoned +2 for 1 body and 1 tail segment.
			{
				...
				catch (Exception e)
				{
					...
					ModContent.GetInstance<RijamsMod>().Logger.Error($"GiantWorm Projectile index out of bounds for drawing the segments. i was {i}; i * 4 {i * 4}; Projectile.oldPos.Length {Projectile.oldPos.Length}; TrailCacheLength was {ProjectileID.Sets.TrailCacheLength[Type]}. {e}");
					Projectile.Kill();
					ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral("Error with the Giant Worm minion. Try summoning it again."), Color.OrangeRed);
				}
				...
				// If the segment is the tail, it gets extra space.
				currentSegment = lastSegmentPos - new Vector2(i == numberOfSegmentsToDraw - 1 ? spaceBetweenSegments + 2 : spaceBetweenSegments, 0f).RotatedBy(rotation, Vector2.Zero);
