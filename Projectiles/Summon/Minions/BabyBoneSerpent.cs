...
{
    public class BabyBoneSerpent : ModProjectile
    {
		...
		public override void AI()
        {
			...
			{
				player.ClearBuff(ModContent.BuffType<Buffs.Minions.BabyBoneSerpentBuff>());
			}
			if (player.HasBuff(ModContent.BuffType<Buffs.Minions.BabyBoneSerpentBuff>()))
			{
		...
		public static void HoverAroundPlayer(Projectile projectile, Player player, ref float minVelocity)
		{
			...
			minVelocity = Math.Min(10f, Math.Max(4f, player.velocity.Length())); // Get faster if the player is moving fast.
