...
{
	public class FrostyGusterProj : ModProjectile
	{
		...
		private static List<Tuple<int, float>> _frostyGusterTargetList = new();
		private static NPCDistanceByIndexComparator _miniGusterTargetComparer = new();

		// Adapted from the Medusa Head
		public override void AI()
		{
			float hitDistance = 12f * 16f; // 8 tiles or 128 pixels
			...
			if (Main.myPlayer == Projectile.owner)
			{
				...
				if (!attacking || Projectile.ai[0] % 10f == 0f)
				{
					...
					if (canAttack && player.CheckMana(player.inventory[player.selectedItem], pay: false)) // Pay mana later
					{
						...
						for (int j = 0; j < Main.maxNPCs; j++)
						{
							...
							if (searchNPC.active && distanceToNPC < hitDistance && searchNPC.CanBeChasedBy(this) && Collision.CanHitLine(searchNPC.position, searchNPC.width, searchNPC.height, projPosition, 0, 0))
								_frostyGusterTargetList.Add(Tuple.Create(j, distanceToNPC));
						}

						_frostyGusterTargetList.Sort(_miniGusterTargetComparer);
						for (int k = 0; k < _frostyGusterTargetList.Count && k < 1; k++) // k < 3
						{
							Tuple<int, float> tuple = _frostyGusterTargetList[k];
							...
							if (player.CheckMana(player.inventory[player.selectedItem], pay: true))
							{
								...
								Projectile gust = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), projPosition, distFromNPCToProj, ModContent.ProjectileType<FrostyWindGust>(), 0, Projectile.knockBack, Projectile.owner, Projectile.whoAmI);
								...
								{
									PositionInWorld = player.Center + positionOffset,
									MovementVector = new Vector2(Projectile.direction, 1f),
									UniqueInfoPiece = 150
								});
